...
import (
	...
	"github.com/aloim/joy4/format/rtsp/sdp"
...
func (self *Client) findRTSP() (block []byte, data []byte, err error) {
	...
	for i := 0; ; i++ {
		...
		if stat == Dollar && len(peek) >= 12 {
			if self.DebugRtp {
				logrus.Info("rtsp: dollar at", i, len(peek))
			}
	...
	*/
	...
	case naluType == 7: // sps
		...
		if len(self.sps) == 0 {
			...
			self.sps = packet
			if self.client != nil && self.client.DebugRtp {
				logrus.Info("rtsp: sps changed")
			}
		}

	case naluType == 8: // pps
		if self.client != nil && self.client.DebugRtp {
			logrus.Info("rtsp: got pps")
		}
		if len(self.pps) == 0 {
			self.pps = packet
			self.makeCodecData()
...
func (self *Stream) handleRtpPacket(packet []byte) (err error) {
	...
	if self.client != nil && self.client.DebugRtp {
		logrus.Info("rtp: packet", self.CodecData.Type(), "len", len(packet))
		dumpsize := len(packet)
		if dumpsize > 32 {
			dumpsize = 32
		}
		logrus.Info(hex.Dump(packet[:dumpsize]))
	}
...
func (self *Client) handleBlock(block []byte) (pkt av.Packet, ok bool, err error) {
	...
	if blockno%2 != 0 {
		if self.DebugRtp {
			logrus.Info("rtsp: rtcp block len", len(block)-4)
		}
		return
	}
	...
	if stream.gotpkt {
		...
		*/
		...
		if self.DebugRtp {
			logrus.Info("rtp: pktout", pkt.Idx, pkt.Time, len(pkt.Data))
		}
